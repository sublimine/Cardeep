...
def ayvens_platform_cdp_code(country_code: str = DEFAULT_COUNTRY) -> str:
...
async def harvest(concurrency: int = DEFAULT_CONCURRENCY) -> dict:
    ...
    try:
        ...
        # A pre-record_run exception (ensure_platform_entity, GeoResolver.load, a setup query)
        ...
        try:
            await record_run(conn, AYVENS_SOURCE_KEY, ok=False, rows=0,
                             error=f"harvest fatal: {type(exc).__name__}: {exc}")
            await auto_repair(conn, AYVENS_SOURCE_KEY, str(exc), phase="scrape")
