...
def mercedes_benz_platform_cdp_code(country_code: str = DEFAULT_COUNTRY) -> str:
...
# P05: OEM VO portal (legal_name/kind class). is_tier1 is parametrized (MB_IS_TIER1).
MB_SPEC = PlatformSpec(
    cdp_code=mercedes_benz_platform_cdp_code(), kind=MB_KIND, legal_name=MB_LEGAL_NAME,
    trade_name=MB_TRADE_NAME, website=MB_WEBSITE, source_key=MB_SOURCE_KEY,
    source_ref=MB_DOMAIN, data_surface="internal_api",
    surface_detail={"endpoint": ENDPOINT, "host": host_of(ENDPOINT), "method": "POST",
                    "page_size": PAGE_SIZE, "denominator": "data.count",
                    "surface_intent": "internal_ajax_list_endpoint",
                    "engine": "curl_cffi/chrome131_impersonate"},
    website_waf=MB_WAF, is_tier1=MB_IS_TIER1, defense_tier=MB_DEFENSE_TIER,
    source_group=MB_SOURCE_GROUP, role=MB_ROLE, family=MB_FAMILY,
...
async def ensure_platform_entity(conn: asyncpg.Connection) -> str:
    """Idempotently ensure the mercedes_benz platform entity + platform_meta exist.
    P05: thin adopter of _core.ensure_platform_entity via MB_SPEC (behaviour preserved)."""
    return await _core_ensure_platform_entity(conn, MB_SPEC)
