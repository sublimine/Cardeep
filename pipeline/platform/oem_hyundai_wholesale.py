...
def hy_platform_cdp_code(country_code: str = DEFAULT_COUNTRY) -> str:
...
# P05: OEM VO portal (legal_name/kind class). is_tier1=TRUE (WAF on the public site).
HY_SPEC = PlatformSpec(
    cdp_code=hy_platform_cdp_code(), kind=HY_KIND, legal_name=HY_LEGAL_NAME,
    trade_name=HY_TRADE_NAME, website=HY_WEBSITE, source_key=HY_SOURCE_KEY,
    source_ref=HY_DOMAIN, data_surface="internal_api",
    surface_detail={"endpoint": ENDPOINT, "host": host_of(ENDPOINT), "method": "GET",
                    "dealer_geo_endpoint": INSTALL_ENDPOINT, "denominator": "len(vehiculos)",
                    "surface_intent": "internal_opencart_json_api",
                    "engine": "curl_cffi/chrome131_impersonate"},
    website_waf=HY_WAF, is_tier1=True, defense_tier=HY_DEFENSE_TIER,
    source_group=HY_SOURCE_GROUP, role=HY_ROLE, family=HY_FAMILY,
...
async def ensure_platform_entity(conn: asyncpg.Connection) -> str:
    """Idempotently ensure the hyundai platform entity + platform_meta exist.
    P05: thin adopter of _core.ensure_platform_entity via HY_SPEC (behaviour preserved)."""
    return await _core_ensure_platform_entity(conn, HY_SPEC)
