...
def spoticar_platform_cdp_code(country_code: str = DEFAULT_COUNTRY) -> str:
...
# P05: OEM VO portal (legal_name/kind class). kind='oem_vo_portal', is_tier1=TRUE (Akamai),
# 0016 axes + family; legacy refreshed all 7 columns on conflict.
SPOTICAR_SPEC = PlatformSpec(
    cdp_code=spoticar_platform_cdp_code(), kind=SPOTICAR_KIND, legal_name=SPOTICAR_LEGAL_NAME,
    trade_name=SPOTICAR_TRADE_NAME, website=SPOTICAR_WEBSITE, source_key=SPOTICAR_SOURCE_KEY,
    source_ref=SPOTICAR_DOMAIN, data_surface="internal_api",
    surface_detail={"endpoint": ENDPOINT, "host": host_of(ENDPOINT), "method": "GET",
                    "page_size": PAGE_SIZE, "denominator": "count.value",
                    "surface_intent": "internal_es_json_api",
                    "engine": "curl_cffi/chrome131_impersonate"},
    website_waf=SPOTICAR_WAF, is_tier1=True, defense_tier=SPOTICAR_DEFENSE_TIER,
    source_group=SPOTICAR_SOURCE_GROUP, role=SPOTICAR_ROLE, family=SPOTICAR_FAMILY,
...
async def ensure_platform_entity(conn: asyncpg.Connection) -> str:
    """Idempotently ensure the spoticar platform entity + platform_meta exist.
    P05: thin adopter of _core.ensure_platform_entity via SPOTICAR_SPEC (behaviour preserved)."""
    return await _core_ensure_platform_entity(conn, SPOTICAR_SPEC)
...
async def _ingest_window(conn: asyncpg.Connection, geo: GeoResolver, geocoder: ProvinceGeocoder,
    ...
    async with conn.transaction():
        ...
        # A4 (SU-A4): PRICE/KM/PHOTO_CHANGE for re-seen vehicles + refresh their row (shared helper, DRY).
        await emit_change_deltas(
...
async def harvest(max_pages: int = DEFAULT_MAX_PAGES,
    ...
    try:
        ...
        # B9 coverage gate: declared_full = count.value from the first page response.
