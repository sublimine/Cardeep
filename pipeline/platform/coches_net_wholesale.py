...
def coches_platform_cdp_code(country_code: str = DEFAULT_COUNTRY) -> str:
...
# P05: this platform's spec for the unified _core.ensure_platform_entity (replaces the 31-line
# hand-copy). is_tier1=TRUE (Imperva); data_surface='internal_api' (schema-valid — the 'json_api'
# intent lives in surface_detail since the platform_meta CHECK rejects that literal).
COCHES_SPEC = PlatformSpec(
    ...
    surface_detail={"endpoint": ENDPOINT, "host": host_of(ENDPOINT), "method": "POST",
                    "category_id": CATEGORY_CARS, "size": PAGE_SIZE,
...
async def ensure_platform_entity(conn: asyncpg.Connection) -> str:
    ...
    Returns the platform entity_ulid. P05: now a thin adopter of the unified
    _core.ensure_platform_entity via COCHES_SPEC — behaviour preserved (parity-verified),
    the 29-way drift killed for this connector."""
    return await _core_ensure_platform_entity(conn, COCHES_SPEC)
...
from pipeline.platform._core.sql import BULK_UPSERT_ENTITY_SOURCE as _BULK_UPSERT_OWNER_SOURCES
...
# A4 delta (SU-A4): PRICE/KM/PHOTO change emission for re-seen vehicles is the SHARED
# pipeline.delta.emit_change_deltas (DRY across connectors); wired in _ingest_window below.
...
async def _ingest_window(conn: asyncpg.Connection, geo: GeoResolver,
    ...
    async with conn.transaction():
        ...
        delta_counts = await emit_change_deltas(
...
async def harvest(max_pages: int = DEFAULT_MAX_PAGES,
    ...
    try:
        ...
        # otherwise skipped record_run → monitoring-dark (green-review connectors HIGH; same class
        # as the wallapop CRIT). Record the failure, then re-raise.
        try:
            await record_run(conn, COCHES_SOURCE_KEY, ok=False, rows=0,
                             error=f"harvest fatal: {type(exc).__name__}: {exc}")
            await auto_repair(conn, COCHES_SOURCE_KEY, str(exc), phase="scrape")
