...
def kia_platform_cdp_code(country_code: str = DEFAULT_COUNTRY) -> str:
...
# P05: OEM VO portal (legal_name/kind class). is_tier1=FALSE (no tier-1 CDN WAF).
KIA_SPEC = PlatformSpec(
    cdp_code=kia_platform_cdp_code(), kind=KIA_KIND, legal_name=KIA_LEGAL_NAME,
    trade_name=KIA_TRADE_NAME, website=KIA_WEBSITE, source_key=KIA_SOURCE_KEY,
    source_ref=KIA_DOMAIN, data_surface="internal_api",
    surface_detail={"endpoint": ENDPOINT, "host": host_of(ENDPOINT), "method": "POST",
                    "page_size": PAGE_SIZE, "denominator": "sum_cluster_total_vehiculos",
                    "partition": "idconcesionario_cluster",
                    "surface_intent": "vendor_async_servlet",
                    "engine": "curl_cffi/chrome131_impersonate"},
    website_waf=KIA_WAF, is_tier1=False, defense_tier=KIA_DEFENSE_TIER,
    source_group=KIA_SOURCE_GROUP, role=KIA_ROLE, family=KIA_FAMILY,
...
async def ensure_platform_entity(conn: asyncpg.Connection) -> str:
    """Idempotently ensure the kia platform entity + platform_meta exist.
    P05: thin adopter of _core.ensure_platform_entity via KIA_SPEC (behaviour preserved)."""
    return await _core_ensure_platform_entity(conn, KIA_SPEC)
