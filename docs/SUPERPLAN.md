...
| **SU-A2** | Descubrir — denominador P | arco β→φ→Chao2 | β sellado (guarda cadenas + B1∘β, gate cero-sobre-fusión); φ con DIRCE; Chao2 ortogonal + cierre saturación; N̂(P) con CI membership-filtered | 🔵 **β SELLADO** (v3 constrained-UF; verdict 1093; S_obs=**38.555**; 74 tests). Over-merge city-name eliminado; chains OK; B1∘β. Residuo 131 multimuni=geo-noise/name-variants (declarado →SU-A6). **Pend: φ(DIRCE)→N_prof, F3 Chao2→N̂(P) con CI.** Hallazgo AS24 kind 511→clasificador |
