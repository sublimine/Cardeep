...
|---|---|---|---|---|
| **SU-A1** | Código único (B1) | confirmar átomo + mejorar | dup<0,1% por ≥2 vías sobre datos vivos; `v_canonical` íntegra; verdict linkado (SU-0.4) | ✅ 0 under-merge host+muni, 0 bosque roto, verdict 640 → **HALLAZGO cluster AS24** (kind mis-class, prov-bug Clicars, 147 cadena sin org-link) → SU-A2. **CAVEAT 06-15 (gate SU-A9)**: 42.259=TECHO; sobre-conteo ~2.300–6.876 por **geocoding-dup** (mismo dealer→muni distinto; **34.904 deep_links bajo >1 canónico**, 4.621 canónicos); real ~35.400–40.000. Clave nombre+muni LIMPIA (0 dup). **SU-B-DEDUP SELLADO** (0027+script reproducible, verdict **1112** quorum 2/3/3): dealer-count corregido **39.874** (−2.385 merges deep_link). Pend: integrar al servido (API sirve 42.259 aún) |
