...
|---|---|---|
| `migrations/0074_market_stat.sql` → tablas `market_stat_run` + `market_stat` [APLICADA Y VERIFICADA — renumerada de 0073, ver F1] | `market_stat_run`: una ejecución del cómputo (id, ventana, versión de metodología, `n_in`, checks pasados, `published BOOLEAN DEFAULT FALSE`). `market_stat`: filas de agregado (run_id, metric_id M1-M10, claves de segmento `make/model/year_band/fuel/province_code`, `n`, `p25/p50/p75` o valor, `window_start/window_end`, `computed_at`). | Espejo del patrón `vehicle_cluster_run` (run + gate antes de servir). **Doctrina MVCC del proyecto: INSERT de run nuevo + DELETE de runs caducados; JAMÁS UPDATE de filas de agregado.** Los endpoints sirven solo el último run `published=TRUE`. |
| `migrations/0077_dgt_transfer.sql` → tablas `dgt_transfer_batch` + `dgt_transfer` [APLICADA Y VERIFICADA — renumerada de 0074, ver F4] | Ingesta cruda de los microdatos mensuales DGT: batch (mes, URL origen, hash del ZIP, filas) + filas normalizadas (fecha transferencia, provincia INE mapeada, marca, modelo, `cod_tipo`). | **[VERIFICADO en F4]** esquema confirmado byte a byte contra el documento oficial + un fichero real descargado. Sin VIN (restringido desde 2025-02-01, confirmado). Provincia requiere mapeo DGT-alfa→INE (hallazgo de F4, no anticipado aquí). |
| `migrations/0078_dgt_corroboration.sql` → tablas `dgt_corroboration_run` + `dgt_corroboration` [APLICADA Y VERIFICADA — renumerada de 0075] | Resultado de la fusión M8 por cohorte (provincia+marca[+modelo]+mes): `gone_count`, `dgt_count`, `ratio`, run_id. | Cohorte, no unidad — límite honesto §3. Filtrado a `cod_tipo='40'` (turismo) — hallazgo de F4. |
...
**F0 — Verdad de volumen (cerrar el hueco #10 del RECON). [HECHO Y VERIFICADO 2026-07-17]**
...
Resultado real (`scripts/f0_market_volume_truth.py`, re-ejecutable): `vehicle` 2.670.827 (available 2.124.671 / gone 546.156); `vehicle_event` 3.749.360 (verificado 2ª vía = `product_stats.events`, coincide exacto); **span real = 35 días** (< 90 → GATE disparado: M3/M4/M7 arrancan en F2 con ventana reducida declarada, nunca fingiendo 90/45/30 días); price NULL 1,32%; make+model+year completo 80,85%; **1 run `vam_verified=TRUE`** (→ GATE resuelto: M1-M10 usan `v_canonical_vehicle`, NO el fallback sin verificar); cohortes `n≥8` = 41.804 de 441.728 (9,46%); dos definiciones de `available_inventory` global documentadas y NO confundidas (estricta 1.523.328 vs COALESCE 1.981.642) — F1 usa la estricta. Detalle completo en `01-market-intelligence-f0.md`.

**F1 — Migración `0074_market_stat` (renumerada de 0073→0074: colisión real con `0073_auth.sql` de un frente paralelo, resuelta por el propio protocolo de numeración) + job `compute_stats.py` con M1 (distribución de precio). [HECHO Y VERIFICADO 2026-07-18]**
...
Resultado real: helper compartido `pipeline/market/cohort.py` (16 tests unitarios puros, 16/16 verde) factorizado per C-1/C-12 de `00-MASTER.md` para que 03/04/06/07/09 lo importen. Universo estricto `v_canonical_vehicle` (F0 §9). Run de producción real `01KXS2KR1PDHPT3KE6X57BR85N`: n_in=1.241.426, 117.652 filas (86.734 provinciales + 30.918 nacionales), `published=FALSE`, cross-check SQL-vs-Python sobre 5 segmentos reales con **divergencia 0,0%** (tolerancia exigida <0,5%), auditoría adicional 0 filas violan `n>=8` ni `p25<=p50<=p75`. 5/5 tests de integración con fixtures sintéticos (rollback verificado, cero fuga a la DB real) + 5/5 cross-check. Detalle completo en `01-market-intelligence-f1.md`.

**F2 — M3, M4, M5, M7, M9, M10 en el mismo job + gate de publicación. [HECHO Y VERIFICADO 2026-07-18]**
...
**Hallazgo de producción real, corregido en esta misma fase (no maquillado):** la primera corrida real escribió 0 filas de M7 — causa raíz: el `canon` estricto (copiado de M1) excluye 408.155 vehículos (15,3%) nunca tocados por el resolver `vehicle_cluster` (último `vam_verified` 2026-06-22); la ventana "reciente" de M7 cae enteramente después de esa fecha → 100% invisible. M3/M4/M5/M9 compartían la misma ceguera parcial sobre actividad reciente. Corregido con un `_COALESCE_CANON_CTE` (mismo patrón ya probado en `entities.py:74`) para las 5 métricas de actividad reciente; M1/M10 mantienen la definición estricta (fotos de stock, no ventanas recientes — F0 ya lo recomendó así). 33/33 tests re-verificados tras el fix (cero regresión). Corrida defectuosa despublicada con nota de auditoría (no borrada); corrida corregida `01KXS6Q4TJKCWM19KKQN2SJ2J1` publicada: 502.749 filas (M1 117.652, M3 27.233, M4 117.652, M5 52, **M7 4.386 — antes 0**, M9 117.652, M10 118.122), gate bootstrap limpio (sin línea base previa tras despublicar la defectuosa), 0 violaciones de `n≥8` ni de monotonía p25≤p50≤p75. Detalle completo en `01-market-intelligence-f2.md`.

**F3 — Router `services/api/routers/market.py` + corrección de `Api.tsx`. [HECHO Y VERIFICADO 2026-07-18]**
...
Resultado real: `GET /market/segments/{make}/{model}/stats` (M1/M3/M4/M5/M7/M9/M10) + `GET /market/provinces/demand` (M5), registrados en `main.py`, cacheados vía `/market/` en `CACHEABLE_PATH_PREFIXES`. M2/M8 deliberadamente ausentes (alcance de F5/F4); M6 declarado como hueco heredado de la numeración de fases de la propia carta (nunca asignado a F0-F5, tampoco a F7 salvo su mitad longitudinal). 11/11 tests de contrato verdes contra el run real publicado en F2, incluido fallback nacional honesto (`fallback_to_national`) para provincia de baja muestra (Soria). `Api.tsx`: catálogo de 6 endpoints ficticios reemplazado por 6 endpoints reales (verificados por `grep` contra los 26 endpoints reales del proyecto), ejemplo de código con datos reales de la corrida publicada. `npx tsc --noEmit` limpio; 107/107 tests de API preexistentes sin regresión. Detalle completo en `01-market-intelligence-f3.md`.

**F4 — Ingesta DGT (`0077` + `ingest_dgt.py`) y corroboración (`0078` + `corroborate.py`, M8). [HECHO Y VERIFICADO 2026-07-18]**
...
Resultado real: esquema DGT confirmado byte a byte contra el documento oficial (69 campos, suma exacta 714 bytes, cero discrepancia). Dos hallazgos no anticipados por la carta, resueltos en la raíz: (1) los códigos de provincia DGT son letras de matrícula histórica, NO los códigos INE de `geo_province` — mapeo `pipeline/market/dgt_provinces.py` construido cruzando el Anexo I oficial + la tabla viva, 52/52 provincias de junio 2026 mapeadas sin huecos; (2) el fichero de transferencias cubre TODO tipo de vehículo (camiones, motos...), no solo turismos — `cod_tipo` añadido al esquema y filtrado a `'40'` (TURISMO) antes de comprometer ningún dato. Ingesta real junio 2026: 396.069 filas, doble descarga con hash SHA256 idéntico, 280.236 turismos. Corroboración M8 real: 89.144 cohortes, ratios de ejemplo (Madrid) entre 25% (Mercedes-Benz) y 66% (Renault) — DGT siempre por debajo de GONE de Cardeep, consistente con la propia tesis de la carta sobre la debilidad del proxy GONE=vendido. Reconciliación honesta contra GANVAM (186.775 turismos de ocasión junio 2026): desviación explicada (censo DGT ≠ ventas mediadas por negocio de GANVAM), no maquillada. Endpoint `GET /market/dgt-corroboration` añadido a `market.py`. 27 tests nuevos (16 parser + 5 corroboración + 6 contrato de router), todos verdes. Detalle completo en `01-market-intelligence-f4.md`.

**F5 — M2 (price-position) + calibración de cortes. [HECHO — 2/3 VERIFICADO, 1/3 BLOQUEO DECLARADO 2026-07-18]**
...
Resultado real: endpoint construido, calculado EN VIVO (no precomputado), cortes publicados en la propia respuesta. Distribución real analizada sobre 1.183.432 vehículos: p25=0,898 p50=1,000 p75=1,127 — los cortes 0,92/1,08 quedan CONFIRMADOS por estar cerca del IQR real (29,4% bajo/38,3% en/32,3% sobre mercado). `03-garage-fleet` y `07-marketing` (los consumidores previstos de K9/C2) verificados por grep: **no existen todavía en el código** — no hay nada con lo que verificar consistencia cross-pilar aún; este motor es, correctamente, el primero. **Bloqueo real declarado, no maquillado**: el gate adversarial formal de modelo caro (Fable 5/Opus) que la doctrina exige para recalibrar METODOLOGÍA no fue ejecutable en este contexto de sesión (sin herramienta para invocar un modelo distinto) — el análisis estadístico completo queda listo en `01-market-intelligence-f5.md` para que ese gate decida en minutos, pero la validación adversarial formal en sí NO se reclama como hecha. 6/6 tests de contrato + 17/17 de F3/F4 sin regresión. Detalle completo en `01-market-intelligence-f5.md`.
