...
- **SIGUIENTE (especificado):** (a) [DRY opcional] consolidar `resolve_entities` al mismo `_country_guard`; (b) DB-gated al arrancar el stack: 3er cinturón mecánico = constraint/trigger en entity_cluster/vehicle_cluster (blueprint análogo a 01-T1.1) + re-cluster con CIF (T2.1-ejecución, dry-run→golden→Ferrari) + T1.1 vam_verified trigger.
