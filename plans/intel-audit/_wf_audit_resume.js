...
const loaded = await agent(
  `Lee el fichero ${REPO}/_resume_input.json (array JSON de empresas) con la tool Read y devuelve {companies:[...]} con name,url,region,subdomain,priority de CADA entrada, verbatim. No omitas ninguna.`,
  { label: 'load-shortlist', phase: 'Cargar', schema: LIST_SCHEMA })
const companies = (loaded && loaded.companies) || []
log(`Shortlist cargado: ${companies.length} empresas a auditar (RESUME 47)`)
phase('Auditoria')
const promptFor = (co) => {
