...
- [ ] **🔑 RECIPE-CRACKER AUTÓNOMO (CENTRAL — corrección owner 2026-06-28: NO es marginal):** equipo/harness que para CADA dealer/plataforma prueba TODAS las alternativas de scraping coherentemente (escalera curl_cffi→Camoufox→browser-stealth→LLM-selector-local→…), **investiga/adapta herramientas nuevas** (repos/GitHub: camoufox/crawl4ai/scrapling/botasaurus/extruct…), hasta crackear, y guarda la **receta verificada + delta** (sample-verify-delete + VAM, cero crudo). Construye SOBRE: `recipe_harness.py` (spine), `recipe_schema.py` (engine next_data|jsonld|css|llm_local), Camoufox (`engine/fetch.py:57`), governor/proxies/fingerprints, `recipe_extract_web.py`/`dealerprobe.py` (schema.org), `delta.py`. Recon `a33d79bd` + diseño `02-RECIPE-CRACKER.md`. **BUILD COMPLETO (dry-run, 44 goldens verde):** orquestador try-all `191a1ad` (escalera barata→cara, 1er VERIFIED gana, razón-por-rung, `RUNG_REGISTRY` extensible, warm-start) + rung css `5f6c69d` (auto-deriva selectores) + rung `llm_local` `6a5a7a4` (LLM-as-selector, valida salida, Ollama €0 owner-gated) + escalera cableada `default_ladder=[json_api(0)→next_data(1)→jsonld(2)→css(4)→llm_local(7)]` + recipe→delta bridge `5afcde5` (5 eventos, reusa `delta.py` connector-agnostic, GONE-falso suprimido). **PENDIENTE-OWNER (puertas):** browser rungs 5-6 (código `engine/tier1/browser.py` existe, deps camoufox/scrapling/nodriver COMENTADAS en requirements → instalar) + fetch real (egress/ToS) + loop-wiring crack→bridge en EXECUTE (cosecha real). El cracker prueba toda la escalera coherentemente hasta el clavo, contra fixtures hoy; contra targets vivos al abrir las puertas.
- [ ] **Resto gated/owner**: pHash población (egress) · OI-7 numerador (DECISIÓN-OWNER) · OI-8 · merge country-2-readiness (cosecha REAL).
