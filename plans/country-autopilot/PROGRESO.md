...
- [ ] **🔑 RECIPE-CRACKER AUTÓNOMO (CENTRAL — corrección owner 2026-06-28: NO es marginal):** equipo/harness que para CADA dealer/plataforma prueba TODAS las alternativas de scraping coherentemente (escalera curl_cffi→Camoufox→browser-stealth→LLM-selector-local→…), **investiga/adapta herramientas nuevas** (repos/GitHub: camoufox/crawl4ai/scrapling/botasaurus/extruct…), hasta crackear, y guarda la **receta verificada + delta** (sample-verify-delete + VAM, cero crudo). Construye SOBRE: `recipe_harness.py` (spine), `recipe_schema.py` (engine next_data|jsonld|css|llm_local), Camoufox (`engine/fetch.py:57`), governor/proxies/fingerprints, `recipe_extract_web.py`/`dealerprobe.py` (schema.org), `delta.py`. Recon `a33d79bd` HECHO + diseño `02-RECIPE-CRACKER.md`. **BUILD:** orquestador try-all `191a1ad` (7/7: escalera barata→cara, 1er VERIFIED gana, razón-por-rung, `RUNG_REGISTRY` extensible, warm-start por rung ganador) + rung css `5f6c69d` (8/8: auto-deriva selectores CSS del patrón de tarjeta) **HECHOS**; rung `llm_local` + integración-escalera (cablear css/llm en default_ladder) + recipe→delta tie **EN VUELO** (`ad3bee43`,`aa371343`). Provisión deps (camoufox/scrapling/patchright/nodriver, comentadas en requirements) + fetch real = **PUERTAS OWNER** (egress/legal/€>0).
