...
$(".app-chat .chat-user ").on("click", function (event) {
    if ($(this).hasClass(".active")) {
        ...
        $(".user-chat-box .chat-user").removeClass("bg-lightgray dark:bg-darkprimary");
...
$(".message-type-box").on("keydown", function (event) {
    if (event.key === "Enter") {
        ...
        $messageHtml =
            '<div class="text-end mb-3"> <div class="py-1.5 px-2 bg-lightinfo dark:bg-darkinfo rounded-md inline-block text-dark dark:text-white text-sm">' +
            chatMessageValue +
            '</div> <div class="text-xs text-link dark:text-darklink mt-2">' +
            time +
            "</div>  </div>";
