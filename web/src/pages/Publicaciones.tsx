...
import {
  Check, X, AlertTriangle, Clock, ExternalLink, RefreshCw, Radio,
} from 'lucide-react'
...
function CoverageHeader({ coverage }: { coverage: PublishingCoverage }) {
  ...
  return (
    <div className="mb-5">
      <h1 className="mb-1 text-[20px] font-bold" style={{ color: 'var(--text-primary)' }}>Tu stock en los portales</h1>
      <p className="mb-4 text-[12px]" style={{ color: 'var(--text-muted)' }}>
        {coverage.total_available === 0
          ...
          : coverage.platforms.length === 0
            ? `Ninguno de tus ${coverage.total_available} coches aparece en un portal todavía.`
            : `Al menos ${inAnyPortal} de tus ${coverage.total_available} coches están en algún portal (${anyListed.toLocaleString()} anuncios activos en total).`}
      </p>

      {coverage.platforms.length > 0 && (
        <div className="grid gap-2.5" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(190px, 1fr))' }}>
          {coverage.platforms.map((p, i) => (
            <motion.div key={p.cdp_code} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}>
              <Card className="!p-3.5">
                ...
                <div className="mb-1 text-[22px] font-extrabold leading-none" style={{ color: BAND_COLOR[p.band] }}>
                  {Math.round(p.coverage_pct * 100)}%
                </div>
  ...
  if (edge.anomaly === 'sold_still_listed') {
    return (
      <div className="flex flex-col gap-0.5">
        <Badge color="red" dot>Vendido, sigue publicado</Badge>
        <a href={edge.listing_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-[10px]" style={{ color: 'var(--text-muted)' }}>
  ...
  return (
    <div className="flex flex-col gap-1">
      <div className="flex items-center gap-1.5">
        ...
        <a href={edge.listing_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-[11.5px] font-medium hover:underline" style={{ color: 'var(--text-primary)' }}>
          Publicado <ExternalLink className="h-2.5 w-2.5 opacity-60" />
        </a>
        {edge.old_listing && (
      ...
      {divergent && edge.platform_price != null && (
        <span className="text-[10.5px]" style={{ color: WARN }}>
    ...
    <Card className="!p-0 overflow-hidden">
      <div className="overflow-x-auto">
        <table className="w-full border-collapse text-left">
          ...
          <tbody>
            {rows.map(row => (
              <tr key={row.vehicle_ulid} style={{ borderBottom: '1px solid var(--border-subtle)' }}>
                <td className="sticky left-0 z-10 p-3 align-top" style={{ background: 'var(--bg-elevated)' }}>
                  <div className="flex items-center gap-2.5">
                    {row.photo_url
                      ? <img src={row.photo_url} alt="" className="h-9 w-12 shrink-0 rounded object-cover" style={{ background: 'var(--bg-surface)' }} />
                      : <div className="h-9 w-12 shrink-0 rounded" style={{ background: 'var(--bg-surface)' }} />}
