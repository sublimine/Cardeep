// web/src/pages/Community.tsx — 08-forum-community F5: "/community", the forum feed
...
import { MessageSquare, TrendingUp, Clock, MapPin, Plus, ShieldCheck } from 'lucide-react'
  ...
  return (
    <div className="mb-6">
      <div className="mb-3 grid gap-3" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))' }}>
        ...
        <Card className="!p-4">
          <div className="text-[11px] uppercase tracking-wide" style={{ color: 'var(--text-muted)' }}>Coincidencias (7d)</div>
          <div className="mt-1 text-[26px] font-extrabold" style={{ color: 'var(--text-primary)' }}>{pulse?.matches_served_7d ?? '—'}</div>
        </Card>
      </div>
      {/* Heatmap simplificado: lista por actividad, no mapa SVG interactivo — decisión de
          alcance declarada (no existe aún un primitivo SpainMap reutilizable en el repo;
          construir uno de cero para esta única pantalla habría sido alcance desproporcionado
          para un pilar Prioridad 2). Misma fuente de datos que un heatmap real usaría. */}
      {top.length > 0 && (
        <div className="flex flex-wrap gap-2">
          {top.map((p) => (
            <span key={p.code} className="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px]" style={{ background: 'var(--bg-surface)', color: 'var(--text-secondary)' }}>
              <MapPin className="h-3 w-3" /> {p.code} · {p.activeThreads}
            </span>
          ))}
        </div>
      )}
...
function ThreadRow({ thread }: { thread: ThreadSummary }) {
  ...
  return (
    <Card className="mb-2 cursor-pointer" onClick={() => navigate(`/community/thread/${thread.thread_ulid}`)} hover>
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            ...
            {thread.verified_anchor_count > 0 && (
              <span className="inline-flex items-center gap-1 text-[10.5px]" style={{ color: '#10b981' }}>
    ...
    <div className="mx-auto p-[20px_24px_48px]" style={{ maxWidth: 900 }}>
      ...
      <div className="mb-3 flex gap-2">
        <button
          ...
          style={{ background: sort === 'hot' ? 'var(--bg-surface)' : 'transparent', color: 'var(--text-primary)', border: '1px solid var(--border-subtle)' }}
        >
          Destacados
        </button>
        <button
          ...
          className="rounded-md px-3 py-1.5 text-[12px] font-medium"
          style={{ background: sort === 'recent' ? 'var(--bg-surface)' : 'transparent', color: 'var(--text-primary)', border: '1px solid var(--border-subtle)' }}
        >
          Recientes
        </button>
      </div>
      ...
      {threads.length === 0 ? (
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          {threads.map((t) => <ThreadRow key={t.thread_ulid} thread={t} />)}
        </motion.div>
      )}
