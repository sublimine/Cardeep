      ...
      >
        <FormCard>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            {/* Email — `type="text"`, not `type="email"`, so the browser's own format
                check does not reject the short `admin` operator account before the
                form is ever submitted. The API never validated the format here
                either: LoginRequest.email is a plain str and the lookup is
                `WHERE email_lower = lower($1)`, so an identifier that is not an
                address resolves fine. Registration still demands a real address
                (RegisterRequest._valid_email). */}
            <Input
              label="Email"
              type="text"
