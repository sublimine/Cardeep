...
export default defineConfig({
  ...
  resolve: {
    alias: {
      // The landing is a verbatim port from the `web/` app, where these imports
      // read `@/...`. `@` is not remapped here (it would collide with this app's
      // own `src/`), so the copied files use `@landing` instead.
      '@landing': fileURLToPath(new URL('./src/landing-v4', import.meta.url)),
    },
  },
  server: {
    allowedHosts: true,
  ...
  build: {
    outDir: 'dist',
    // Distinct from the landing app's default `assets/` so both can be
    // reverse-proxied under the same domain (cardeep.vercel.app) without
    // hashed-filename collisions between the two independent SPAs.
